...
# # ===== replace your developerTest with this =====
# def developerTest(
#     image_path: str,
...
#         # === use remapped number for display and naming ===
#         nozzle_num_disp = _remap_nozzle_num(nozzle_num)

#         # Draw YOLO bbox and label (use remapped number)
#         cv2.rectangle(vis, (x1, y1), (x2, y2), cyan, 2)
#         cv2.putText(
#             vis, f"#{nozzle_num_disp}", (x1, max(0, y1 - 6)),
...
#         # Collect blocked quadrant names (use remapped number)
#         for q_idx, is_blocked in enumerate((s0, s1, s2, s3)):
#             if is_blocked:
#                 blocked_names.append(f"nozzle{nozzle_num_disp}{_QUAD_NAMES[q_idx]}")
